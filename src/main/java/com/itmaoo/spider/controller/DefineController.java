...
import com.itmaoo.spider.dao.ColumnDefineDao;
...
public class DefineController {
	...
	public void saveProvider(@RequestBody DataProviderView dataProviderView) {
		...
		String json = SpiderStrings.fetchRegexMidle(dataProviderView.getFetchRegexMidle(),dataProviderView.getIndex(), response);
	...
	public void saveTable(TableDefineView tableDefineView) {
		...
		TableDefinePo tableDefinePo = new TableDefinePo();
		
		tableDefinePo.setChecked(tableDefinePo.getChecked());
		tableDefinePo.setFetchInternalMax(tableDefinePo.getFetchInternalMax());
		tableDefinePo.setFetchInternalMin(tableDefinePo.getFetchInternalMin());
		tableDefinePo.setParams(tableDefinePo.getParams());
		tableDefinePo.setTableCode(tableDefineView.getTableCode());
		tableDefinePo.setTableName(tableDefineView.getTableName());
		
		tableDefineDao.saveTable(tableDefinePo);
		
		for(TableColumnView row:tableDefineView.getRowDefine()) {
			ColumnDefinePo columnDefinePo =  new ColumnDefinePo();
			columnDefinePo.setColumnName(row.getColumnName());
			columnDefinePo.setFetchRegex(row.getFetchRegex());
			columnDefinePo.setFetchType(row.getFetchType());
			columnDefinePo.setIndex(row.getIndex());
			columnDefinePo.setPath(row.getPath());
			columnDefinePo.setProviderId(row.getProviderId());
			columnDefineDao.save(columnDefinePo);
		}
	...
	public Map<String,String> saveTableCheck(@RequestBody TableDefineView tableDefineView) {
		...
		for(TableColumnView row:tableDefineView.getRowDefine()) {
			...
			String json = SpiderStrings.fetchRegexMidle(dataProviderPo.getFetchRegexMidle(), dataProviderPo.getIndex(),response);
			...
			String value = SpiderStrings.fetchRegexMidle(row.getFetchRegex(), row.getIndex(),response);

			map.put(row.getColumnName(), value);
		}
	...
	public static void main(String[] args) {
		...
		String value = SpiderStrings.fetchRegexMidle("[*]total:\"[*]\"[*]", 3,test);
		String value2 = JsonUtils.getSubString(test2, "/total");
